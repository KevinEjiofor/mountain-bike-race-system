...
const raceSchema = Joi.object({
    ...
    categories: Joi.array()
        .items(Joi.string().valid('Professional', 'Amateur', 'Youth'))
        .min(1)
        .default(['Amateur'])
        .messages({
    ...
    status: Joi.string()
        .valid('Draft', 'Open', 'Closed', 'InProgress', 'Completed', 'Cancelled')
        .default('Draft')
        .messages({
...
const raceResultSchema = Joi.object({
    ...
    status: Joi.string()
        .valid('Registered', 'Started', 'Finished', 'DNF', 'DSQ')
        .default('Registered')
        .messages({
...
const riderSchema = Joi.object({
    ...
    category: Joi.string()
        .valid('Professional', 'Amateur', 'Youth')
        .default('Amateur')
        .messages({
...
const validateRace = (data) => {
    return raceSchema.validate(data, { abortEarly: false });
};

const validateRaceResult = (data) => {
    return raceResultSchema.validate(data, { abortEarly: false });
};

const validateRider = (data) => {
    return riderSchema.validate(data, { abortEarly: false });
};

// Update validation (for PATCH requests)
const validateRaceUpdate = (data) => {
    const updateSchema = raceSchema.fork(
        ['name', 'location', 'startTime', 'distance'],
...
const validateRaceResultUpdate = (data) => {
    const updateSchema = raceResultSchema.fork(
        ['rider', 'race', 'startTime'],
        (schema) => schema.optional()
    );
    return updateSchema.validate(data, { abortEarly: false });
};
...
const validateRiderUpdate = (data) => {
    const updateSchema = riderSchema.fork(
        ['firstName', 'lastName', 'email', 'dateOfBirth', 'nationality', 'bikeType'],
        (schema) => schema.optional()
    );
    return updateSchema.validate(data, { abortEarly: false });
};
